...
namespace fastdee.Stratum.Tests
{
    public class HeaderGeneratorTests
    {
        ...
        public void MatchesGoldenCanonWithMerkles(byte[] expected, byte[] n1, ushort n2sz, ulong nonce2)
        {
        ...
        static NewJob ObservedJob()
        {
            var jobid = "not_really_relevant";
            var version = new byte[] { 0x20, 0x00, 0x00, 0x00 }; // version
            var nbits = new byte[] { 0x1a, 0x0e, 0xde, 0xd9 }; // nbits - network difficulty
            var ntime = new byte[] { 0x5f, 0xdf, 0x98, 0x11 }; // ntime
            byte[] trie = null;
            var coinhead = new byte[] {
            ...
            var pack = new NewJob(jobid, version, trie, coinhead, cointail, nbits, ntime, true);
